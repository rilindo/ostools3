...
class OSTools:
    ...
    def volumes_by_instanceid(self, instid):
        querystr = "SELECT created_at,id,user_id,project_id,size,instance_id,status,attach_status,display_name,display_description \
                    FROM volumes \
                    WHERE deleted=0 AND instance_id='%s'" % (instid)

        results = self.__query(querystr, 'volumes_by_instanceid', 'novadb')
        return results
    ...
    def volumes_by_tenantid(self, tenantid):
        ...
        results = self.__query(querystr, 'volumes_by_tenantid', 'novadb')
        return results
