...
# Output
if not quiet:
   print("%s%-6s %-10s  %-6s  %-36s  %-9s  %s%s" % (bld,"CNODE","INSTANCE","TENANT","UUID","STATE","HOSTNAME",nrm))
for row in vmlist:
    instance_id,host,project_id,uuid,vm_state,hostname = row
    print("%-6s i-%08x  %-6s  %-36s  %-9s  %s" % (host,instance_id,project_id,uuid,vm_state,hostname))
